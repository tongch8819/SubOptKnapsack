...
if __name__ == "__main__":
    ...
    # facebook.bv = np.array([10, 10, 10, 10])
    #
    # S, upb = MWU(facebook, upb='ub0')
    # print(f"S:{S}, value:{facebook.objective(S)}, upb:{upb}")
    ...
    for seed in range(0, 200):
        for budget in range(6, 20):
            ...
            model = model_factory(task, n, seed, budget, cm="normal", knap=True)
            model.bv = np.array([budget] * 4)

            S, upb, w = MWU(model, upb='ub0')
            ...
            save_path = os.path.join(save_dir, "{}-{}-{}-{:.2f}.pckl".format(
                "mwu", "ub0A", model.__class__.__name__, budget))
