...
def compute_image_sum(root_dir, skip_mode=False):
    ...
    for budget in bds:
        model = ImageSummarization(
            image_path="dataset/image/500_cifar10_sample.npy", budget=budget, max_num=max_num)
        for up in upper_bounds:
            for algo in algos:
                save_path = os.path.join(root_dir, "{}-{}-{}-{:.2f}.pckl".format(
                    algo, up, model.__class__.__name__, budget))
    ...
    # budget = 10.0
    # model = ImageSummarization(
    #     image_path="dataset/image/500_cifar10_sample.npy", budget=budget)
...
def compute_movie_recom(root_dir, skip_mode=False):
    # budget = 30.0
    # model = MovieRecommendation(matrix_path="dataset/movie/user_by_movies_small_rating.npy", budget=budget, k = 30, n = 50)
...
def compute_revenue_max(root_dir, skip_mode=False):
    # budget = 1
    # model = RevenueMax(budget=budget, pckl_path="dataset/revenue/25_youtube_top5000.pkl")
    # costs = model.costs_obj
...
if __name__ == "__main__":
    ...
    parser.add_argument("task_num", type=int, help="0,1,2,3")
